...
public class BankAccountServiceImplementation implements BankAccountService {
	...
	public CustomerDTO updateCustomer(CustomerDTO customerDTO) {
		Customer customer = dtoMapper.fromCustomerDTO(customerDTO);
		AppUser appUser = new AppUser(null, customerRepository.findById(customer.getId()).get().getName(), customer.getName(), new ArrayList<>());
		System.out.println(customer+" "+appUser);
		customer = customerRepository.save(customer);
		accountService.update(appUser);
		accountService.addRoleToUser(customer.getName(), "CUSTOMER");
		System.out.println("After the save " +customer+" "+appUser);
		return dtoMapper.fromCustomer(customer);
	}
	...
	public void deleteCustomer(Long customerId) {
		Customer cs = customerRepository.findById(customerId).get();
		customerRepository.deleteById(customerId);
		AppUser au = appUserRepository.findByUsername(cs.getName());
		accountService.deleteUser(au.getId());
	}
	...
	public AccountHistoryDTO getAccoutHistory(String accountId, int page, int size) throws BankAccountNotFound {
		BankAccount bankAccount = bankAccountRepository.findById(accountId).orElseThrow(()->
		 new BankAccountNotFound("bank not found!! "));
